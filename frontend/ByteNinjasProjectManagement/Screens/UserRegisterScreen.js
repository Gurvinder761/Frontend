...
import { cyan } from 'google-material-color-palette-json';
...
export default function UserRegisterScreen({navigation}) {
    ...
    //new
    const [IsAdmin,ChangeToggle] = useState(false);
    const [Messege,ChangeMessege] = useState('Admin Login');
    const [ButtonText,ChangeButtontext] = useState("I'm a member");
    ...
    //Admin Button Toggle
    const AdminButtonToggle = () => {
        if(!IsAdmin){
            ChangeToggle(true);
            ChangeMessege('Member Login');
            ChangeButtontext("I'm an Admin");
            console.log(IsAdmin);
        }
        if(IsAdmin){
            ChangeToggle(false);
            ChangeMessege('Admin Login');
            ChangeButtontext("I'm a Member");
            console.log(IsAdmin);
        }
    }
    ...
    return (
        ...
        <CustomButton style={styles.btn} title={ButtonText} onPress={AdminButtonToggle}/>
